...
{
  // int id = ++thread_count;
  int id = knowledge.evaluate ("++thread_count");
  int last =  id - 1;
  ...
  if (id == knowledge.get ("max_threads"))
  {
...
int ACE_TMAIN (int argc, ACE_TCHAR * argv[])
{
  ...
  for (int i = 0; i < knowledge.get ("max_threads"); i++)
  {
...
int parse_args (int argc, ACE_TCHAR * argv[])
{
  ...
  while ((option = cmd_opts ()) != EOF)
  {
    ...
    case 't':
      // thread number
      knowledge.set ("max_threads", ACE_OS::atoi(arg));
      break;
    case ':':
